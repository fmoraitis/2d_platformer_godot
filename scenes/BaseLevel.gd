...
func on_players_dashing_kill(dashing_direction:int, enemy:CharacterBody2D):
	...
	enemy_death_scene_init(dashing_direction, enemy)
	enemy.queue_free()
	
func enemy_death_scene_init(dashing_direction:int, enemy:CharacterBody2D):	
	...
	enemy_death_scene_node.velocity.x = -1*  enemy.velocity.x
#	enemy_death_scene_node.velocity.y = -100	
...
func on_player_died(enemy_direction,type_of_enemy:ENEMY_IS):
	player_death_scene_init(enemy_direction,type_of_enemy)
	player_instance.queue_free()
	await get_tree().create_timer(2.5).timeout
	add_child(level_game_over_screen.instantiate())

func player_death_scene_init(enemy_type_or_direction:int, type_of_enemy:ENEMY_IS):
